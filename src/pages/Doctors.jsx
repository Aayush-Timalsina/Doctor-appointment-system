...
const Doctors = () => {
  ...
  //Filtering the doctors to be displayed in all doctor tab
  const applyFilter = () => {
    if (speciality) {
          ...
          <div className='w-full grid grid-cols-auto gap-4 gap-y-6'>
            {
              filterDoc.map((item, index) => (
                <div onClick={() => navigate(`/appointments/${item._id}`)} className='border rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500' key={index}>
